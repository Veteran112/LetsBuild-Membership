...
const PaymentForm = () => {
  ...
  useEffect(() => {
    ...
    switch (licenseplan) {
      ...
      case 'premier':
        setPlan('Premier')
        if (isAnnual) _price = 79.9
        else _price = 99.0

        break
      default:
        if (isAnnual) _price = 54.0
        else _price = 4.99
    }
  ...
  const handleSubmitNewPayment = async (e) => {
    ...
    try {
      ...
      const data = await createPaymentV2(
        userId,
        plan,
        type,
        sum,
      ...
      if (data.data.message === 'Payment successful') {
        ...
        localStorage.setItem(
          'paymentData',
          JSON.stringify(data.data.sendData.data),
        )
        window.location.href = '/getTicket'
      }
    } catch (error) {
            ...
            <div>
              ...
              <div className="row">
                <div className="col-6 pt">
                  ...
                  <hr className="hr3" />
                  <p className="hdng"> Period </p> {renderNamesOfPassenger()}
                ...
                <div className="col-6">
                  ...
                  <p> {type} </p> <p> {price.toFixed(2)} </p>{' '}
                  <p> {tax.toFixed(2)} </p> <hr className="hr3" />
                  <p> {sum.toFixed(2)}</p>{' '}
